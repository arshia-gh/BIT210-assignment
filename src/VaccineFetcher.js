import {vaccines} from "./PCVS";//} from "./PCVS";
...
function x () {
    function AddNewVaccine() {
        ...
        if (vaccineName && manufacturer) {
          ...
          const newVaccine = new Vaccine(getNextVaccineID(), vaccineName, manufacturer);
          vaccines.push(newVaccine);
          AppendVaccineToTable(newVaccine);
    
          alert(`${vaccineName} made by ${manufacturer} has been added`);
          form.reset();
        }
        // if(vaccines.find)
      }
      ...
      vaccineTable.addEventListener('click', (e) => {
        ...
        if(vaccineID) 
        {
          let selectedVaccine = vaccines.find(vaccine => vaccine.vaccineID === vaccineID);
          
          if(selectedVaccine)
            showAddBatchModal(selectedVaccine);
      ...
      function showAddBatchModal(vaccine) {
        ...
        const modal = new bootstrap.Modal(document.getElementById('addNewBatchModal'), {});
        document.getElementById('addNewBatchModalLabel').innerHTML = `${vaccine.vaccineName} | New Batch`;
        modal.show();
      }
