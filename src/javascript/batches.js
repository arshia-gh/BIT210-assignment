...
addBatchForm.onsubmit = async (e) => {
	...
	const vaccineResult = await request(RESOURCE.VACCINE, {
		query: { vaccineID: vaccineID }
	});
	const vaccine = await vaccineResult[0];

	centre
		.createBatch(batchNo, expiryDate, vaccine, quantityAvailable)
		.then((res) => {
			//res will be the batch object this point
			const batch = {
				batchNo: res.batchNo,
				expiryDate: res.expiryDate,
				noOfPendingAppointment: 0
			};
